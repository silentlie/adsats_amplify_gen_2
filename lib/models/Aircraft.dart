...
class Aircraft extends amplify_core.Model {
  ...
  AircraftModelIdentifier get modelIdentifier {
      return AircraftModelIdentifier(
        id: id
      );
  }
  ...
  const Aircraft._internal({required this.id, required name, required archived, description, staff, document, notices, createdAt, updatedAt}): _name = name, _archived = archived, _description = description, _staff = staff, _document = document, _notices = notices, _createdAt = createdAt, _updatedAt = updatedAt;
  
  factory Aircraft({String? id, required String name, required bool archived, String? description, List<AircraftStaff>? staff, List<AircraftDocument>? document, List<AircraftNotice>? notices}) {
    return Aircraft._internal(
      ...
      document: document != null ? List<AircraftDocument>.unmodifiable(document) : document,
      notices: notices != null ? List<AircraftNotice>.unmodifiable(notices) : notices);
  }
  ...
  Aircraft copyWith({String? name, bool? archived, String? description, List<AircraftStaff>? staff, List<AircraftDocument>? document, List<AircraftNotice>? notices}) {
    return Aircraft._internal(
      id: id,
      name: name ?? this.name,
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'name': _name, 'archived': _archived, 'description': _description, 'staff': _staff?.map((AircraftStaff? e) => e?.toJson()).toList(), 'document': _document?.map((AircraftDocument? e) => e?.toJson()).toList(), 'notices': _notices?.map((AircraftNotice? e) => e?.toJson()).toList(), 'createdAt': _createdAt?.format(), 'updatedAt': _updatedAt?.format()
  };
  ...
  static final amplify_core.QueryModelIdentifier<AircraftModelIdentifier> MODEL_IDENTIFIER = amplify_core.QueryModelIdentifier<AircraftModelIdentifier>();
 ...
 */
class AircraftModelIdentifier implements amplify_core.ModelIdentifier<Aircraft> {
  final String id;

  /** Create an instance of AircraftModelIdentifier using [id] the primary key. */
  const AircraftModelIdentifier({
    required this.id});
  ...
  bool operator ==(Object other) {
    ...
    return other is AircraftModelIdentifier &&
      id == other.id;
  }
